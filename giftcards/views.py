...
def add_new_card_ajax(request):
    ...
    exists = True
    try:
        giftcard = Giftcard.objects.get(card_id=card_id)
...
def redeem_card(request, transaction_id):
    ...
    if request.META.has_key('HTTP_USER_AGENT'):
        ...
        if b or v:
            return HttpResponseRedirect("/giftcards/redeem-wholefoods-giftcard-mobile/" + transaction_id)
            
    return HttpResponseRedirect("/giftcards/redeem-wholefoods-giftcard-print/" + transaction_id)
    
def redeem_card_mobile(request, transaction_id):
    transaction = Transaction.objects.get(id_slug=transaction_id)
    giftcard = Giftcard.objects.get(transaction=transaction)
    return render(
        request, 
        "mobile-giftcard.html", 
        { "giftcard" : giftcard }
    )

def redeem_card_print(request, transaction_id):
    transaction = Transaction.objects.get(id_slug=transaction_id)
    giftcard = Giftcard.objects.get(transaction=transaction)
    return render(
        request, 
